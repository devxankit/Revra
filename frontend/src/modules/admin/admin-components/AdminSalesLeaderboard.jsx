...
const AdminSalesLeaderboard = () => {
  ...
  const transformMember = (member) => {
    ...
    const conversionRate =
      member.conversionRate || (leads > 0 ? Math.round((conversions / leads) * 100) : 0)
  ...
  const loadData = async () => {
    try {
      ...
      const sales = rawSales.map(transformMember)
      setSalesData(sales)

      const totalMembers = sales.length
      const totalRevenue = sales.reduce(
        (sum, m) => sum + (m.salesMetrics?.revenue || 0),
        0
      )
      const totalCompleted = sales.reduce(
        (sum, m) => sum + (m.salesMetrics?.conversions || 0),
        0
      )
      const totalDeals = sales.reduce(
      ...
      const avgScore =
        totalMembers > 0
          ? Math.round(
              sales.reduce((sum, m) => sum + (m.salesMetrics?.revenue || 0), 0) /
                totalMembers
            )
          : 0
      const avgCompletionRate =
        totalMembers > 0
          ? Math.round(
              sales.reduce((sum, m) => sum + (m.conversionRate || 0), 0) / totalMembers
            )
          : 0
      const topPerformer =
        totalMembers > 0
          ? sales.reduce(
              (top, m) =>
                (m.salesMetrics?.revenue || 0) > (top.salesMetrics?.revenue || 0) ? m : top,
              sales[0]
            )
          : null
      ...
      console.error('Error loading admin sales leaderboard:', err)
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <button
          ...
          className={`px-4 py-2 border border-gray-300 rounded-lg flex items-center gap-2 text-sm ${
            showFilters ? 'bg-primary/10 text-primary border-primary/40' : 'hover:bg-gray-50'
          }`}
        >
...
export default AdminSalesLeaderboard
