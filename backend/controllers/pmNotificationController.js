...
const formatRequestNotification = (request, pmIdStr) => {
  const recipientId = extractId(request.recipient);
  const requesterId = extractId(request.requestedBy);

  const isRecipient =
    request.recipientModel === 'PM' && recipientId === pmIdStr;
  const isRequester =
    request.requestedByModel === 'PM' && requesterId === pmIdStr;
...
const formatTaskNotification = (task) => {
  return {
    ...
    message: `A new task "${task.title}" has been created${task.priority === 'urgent' || task.priority === 'high' ? ` with ${task.priority} priority` : ''}`,
...
const formatProjectNotification = (project) => {
  return {
...
// @desc    Get notifications feed for the authenticated PM
// @route   GET /api/pm/notifications
// @access  PM only
const getPMNotifications = asyncHandler(async (req, res, next) => {
  const pmId = req.pm?.id || req.user?.id;
  const pmObjectId = toObjectId(pmId);

  if (!pmObjectId) {
    return next(new ErrorResponse('PM context not found', 401));
  }

  const pmIdStr = pmObjectId.toString();
  ...
  const pmProjects = await Project.find({ projectManager: pmObjectId })
    .select('_id name status progress')
    .lean();

  const projectIds = pmProjects.map((project) => project._id);
  const projectMap = pmProjects.reduce((map, project) => {
  ...
  const requestDocs = await Request.find({
    $or: [
      { recipientModel: 'PM', recipient: pmObjectId },
      { requestedByModel: 'PM', requestedBy: pmObjectId }
    ]
  })
  ...
  const requestNotifications = requestDocs.map((request) =>
    formatRequestNotification(request, pmIdStr)
  );

  // Get project notifications (recently updated projects)
  const recentProjects = await Project.find({
    projectManager: pmObjectId,
    updatedAt: { $gte: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000) } // Last 7 days
  })
    .select('_id name status progress updatedAt createdAt')
    .sort({ updatedAt: -1 })
    .limit(10)
    .lean();

  const projectNotifications = recentProjects.map(formatProjectNotification);

  // Get task notifications (recently created tasks in PM's projects)
